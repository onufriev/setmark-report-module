...
Этот раздел применяется только при выбранной ветке `VISUAL_AND_WORKING` и
определяет стек экспериментальной реализации, а не production-архитектуру.
...
1. зарегистрировать решение через `tools/record_prototype_data_source.py`;
2. перенести тип источника, расположение, команды наполнения и проверки в `prototype/PROTOTYPE-RUNBOOK.md` и `prototype/prototype-manifest.json`.

Источник данных рабочего прототипа не является продуктовым требованием и не
изменяет зафиксированную версию продуктовых сценариев.
