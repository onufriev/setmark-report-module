...
SCHEMA_FILES = {
    ...
    'product/prototype-exemption.json': 'schemas/prototype-exemption.schema.json',
    ...
    if rel == 'product/evidence-register.json':
        ...
        for index, item in enumerate(obj.get('requirements', []), start=1):
            if not isinstance(item, dict):
                ...
                continue
            errors.extend(_required(item, ('id', 'title', 'kind', 'file', 'line', 'domainId', 'domainIds', 'status', 'horizon'), f'requirements[{index}]'))
    elif rel == 'product/open-questions.json':
        for index, item in enumerate(obj.get('questions', []), start=1):
            if not isinstance(item, dict):
                errors.append(f'questions[{index}]: запись должна быть объектом')
                continue
            errors.extend(_required(item, ('id', 'domainId', 'domainIds', 'title', 'file', 'line', 'owner', 'neededFor', 'status'), f'questions[{index}]'))
    return errors
