...
Инструмент не создаёт требование автоматически. После регистрации встроите смысл
решения в соответствующий сценарий, правило, границу или требование к качеству,
удалите закрытый открытый вопрос и добавьте служебную ссылку в невидимый
`product-compiler-requirement` комментарий.

### Выбор необязательного прототипа

```text
python tools/record_prototype_plan.py --plan NONE --message "Передаём требования без прототипа"
python tools/record_prototype_plan.py --plan VISUAL_ONLY --message "Проверяем визуальный прототип"
python tools/record_prototype_plan.py --plan VISUAL_AND_WORKING --message "Проверяем визуальный и рабочий прототипы"
```

План выбирается до завершения PRODUCT_DEFINITION. Стек и источник данных нужны
только для `VISUAL_AND_WORKING`.
...
Новый формат использует понятные документы «Сценарий N», «Правило N» и
«Качество N». Служебные поля находятся в невидимых комментариях
`product-compiler-requirement` и `product-compiler-question`. Пример приведён в
`requirements/README.md`.

Старые `REQ-*` и `NEEDS_INPUT` продолжают читаться для совместимости. Новые
`REQ-DEC-*` запрещены. Каталог областей является контрольным перечнем, а не
обязательной анкетой: PRODUCT_DEFINITION блокируют только видение, границы,
основной сценарий и условия его готовности.
