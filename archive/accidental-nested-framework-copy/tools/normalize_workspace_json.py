...
def normalize_special_document(rel: str, doc: dict) -> dict:
    if rel == 'product/ui-source.json':
        ...
        doc.setdefault('qualityWarnings', [])
        doc.setdefault('diagnostics', [])
    elif rel == 'product/completeness-report.json':
        doc.setdefault('domains', [])
        doc.setdefault('gates', {})
    elif rel == 'product/prototype-exemption.json':
        if not doc.get('plan'):
            doc['plan'] = 'VISUAL_ONLY' if doc.get('status') == 'SKIPPED_BY_PM' else 'NOT_SET'
        doc.pop('status', None)
        doc.setdefault('decision', None)
        doc.setdefault('evidenceRef', None)
        doc.setdefault('approvedAt', None)
...
def ensure_domain_files() -> None:
    # Начиная с версии 5.0 области являются контрольным каталогом, а не набором
    # обязательных файлов. Читаемые документы организованы по сценариям.
    return None
