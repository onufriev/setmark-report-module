...
def core_suite() -> None:
    with tempfile.TemporaryDirectory(prefix=f'product-compiler-{PRODUCT_VERSION}-core-') as temp:
        ...
        requirement_file = workspace / 'requirements/product-vision.md'
        requirement_file.write_text(
            '<!-- product-compiler-requirement: '
            '{"id":"VISION-001","kind":"VISION","domains":["productContext","successSignal"],'
            '"status":"Подтверждено источником","horizon":"CURRENT","evidence":["SRC-EV-001"]} -->\n'
            '# Видение продукта\n\n'
            'Продукт показывает проверяемый основной сценарий и позволяет оценить его пользу.\n',
            encoding='utf-8',
        )
        assert_ok(run(workspace, 'sync_workspace.py', '--verbose'), 'scenario requirement parser and sync')
        index = read_json(workspace / 'product/requirements-index.json')
        requirement_ids = [item.get('id') for item in index.get('requirements', [])]
        assert 'VISION-001' in requirement_ids
        ...
        recorded_question = next(item for item in questions.get('questions', []) if item.get('id') == 'Q-002')
        assert recorded_question.get('status') == 'ANSWERED_NOT_APPLIED'
        assert 'PM-DEC-001' not in (workspace / 'requirements/scope.md').read_text(encoding='utf-8')
        assert not any('REQ-DEC-' in f.read_text(encoding='utf-8') for f in (workspace / 'requirements').rglob('*.md'))

        assert_ok(run(
            workspace,
            'record_pm_decision.py',
            '--question-id', 'Q-003',
            '--domain-id', 'functionalRequirements',
            '--decision', 'Пользователь открывает список объектов и видит их состояние.',
            '--message', 'Основной сценарий — просмотр состояния объектов.',
        ), 'main scenario decision registration')
        (workspace / 'requirements/scope.md').write_text(
            '<!-- product-compiler-requirement: '
            '{"id":"SCOPE-001","kind":"SCOPE","domains":["goalsScope"],'
            '"status":"Подтверждено Product Manager","horizon":"CURRENT","evidence":["PM-DEC-001"]} -->\n'
            '# Границы продукта\n\nТекущая версия проверяет основной пользовательский сценарий.\n',
            encoding='utf-8',
        )
        (workspace / 'requirements/scenarios/001-main-scenario.md').write_text(
            '<!-- product-compiler-requirement: '
            '{"id":"SCN-001","kind":"SCENARIO","domains":["functionalRequirements","mainJourney","acceptanceCriteria"],'
            '"status":"Подтверждено Product Manager","horizon":"CURRENT","evidence":["PM-DEC-002"]} -->\n'
            '# Сценарий 1. Просмотр состояния объектов\n\n'
            '## Цель\n\nПонять текущее состояние объектов.\n\n'
            '## Пользователь\n\nОсновной пользователь продукта.\n\n'
            '## Основной ход\n\n1. Пользователь открывает список.\n2. Продукт показывает состояния.\n\n'
            '## Результат\n\nПользователь видит требующие внимания объекты.\n\n'
            '## Условия готовности\n\n- Состояние каждого доступного объекта понятно.\n',
            encoding='utf-8',
        )
        assert_ok(run(workspace, 'sync_workspace.py', '--strict-json'), 'completed product definition synchronization')
        completeness = read_json(workspace / 'product/completeness-report.json')
        assert completeness.get('gates', {}).get('PRODUCT_DEFINITION', {}).get('status') == 'PASSED'
        assert any(not item.get('blocking') for item in read_json(workspace / 'product/open-questions.json').get('questions', []))
        assert_ok(run(
            workspace,
            'record_prototype_plan.py',
            '--plan', 'NONE',
            '--message', 'Передаём требования без прототипа.',
        ), 'optional prototype plan registration')
        assert_ok(run(workspace, 'commit_requirements.py'), 'requirements baseline with non-blocking questions')
        assert_ok(run(workspace, 'verify_requirements_baseline.py'), 'requirements baseline verification')
