...
from common import PHASE_SEQUENCE, active_phase_sequence, artifact_sha256, load, now, project_path, prototype_plan, run_tool, save, sha256
...
def main() -> int:
    ...
    if args.phase == 'PRODUCT_DEFINITION':
        product_gate = completeness.get('gates', {}).get('PRODUCT_DEFINITION', {})
        if product_gate.get('status') != 'PASSED' or product_gate.get('blockingQuestions'):
            raise SystemExit('PRODUCT_DEFINITION не завершён: не описаны обязательные части продуктового видения')
        if prototype_plan() == 'NOT_SET':
            raise SystemExit('Перед завершением PRODUCT_DEFINITION выберите необязательную ветку прототипирования через record_prototype_plan.py')
    ...
    if required_artifact and args.phase in active_phase_sequence():
