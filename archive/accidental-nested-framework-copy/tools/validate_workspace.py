...
required_files = [
    ...
    'product/prototype-exemption.json',
    ...
    'schemas/prototype-exemption.schema.json',
    ...
    'tools/record_prototype_plan.py',
]
...
for position, requirement in enumerate(index.get('requirements', []), start=1):
    ...
    if status in closed_statuses:
        ...
        for ref in refs:
            ...
            elif not any(evidence_applies_to(evidence, domain) for domain in domains):
                errors.append(f'{requirement_id}: основание {ref} не относится к областям материала')
...
# Решение Product Manager сначала получает статус ANSWERED_NOT_APPLIED. Затем
# Product Compiler встраивает его смысл в сценарий и удаляет открытый вопрос.
...
for review in reviews:
    ...
    phases = active_phase_sequence()
    if review_phase not in phases:
        errors.append(f'{review_id}: отчёт относится к пропущенной ветке прототипирования')
        continue
    next_index = phases.index(review_phase) + 1
    expected_next = phases[next_index] if next_index < len(phases) else None
...
for phase_name in active_phase_sequence():
    if phase_name in approved_phases:
        phases = active_phase_sequence()
        prior = phases[:phases.index(phase_name)]
...
active_phases = active_phase_sequence()
if phase in active_phases and active_phases.index(phase) > active_phases.index('PRODUCT_DEFINITION'):
    baseline_ok, baseline_errors = verify_baseline()
    if not baseline_ok:
        errors.extend('Базовая линия: ' + item for item in baseline_errors)
if prototype_plan() in {'VISUAL_ONLY', 'VISUAL_AND_WORKING'} and phase in active_phases and active_phases.index(phase) >= active_phases.index('VISUAL_VALIDATION'):
...
if prototype_plan() == 'VISUAL_AND_WORKING' and phase in active_phases and active_phases.index(phase) >= active_phases.index('WORKING_VALIDATION'):
