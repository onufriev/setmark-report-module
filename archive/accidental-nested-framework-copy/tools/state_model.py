...
from common import PHASE_SEQUENCE, PRODUCT_VERSION, ROOT, SCHEMA_VERSION, active_phase_sequence, load, prototype_plan
...
def _requirements_status() -> str:
    ...
    if baseline.get('status') == 'COMMITTED' and completeness.get('gates', {}).get('PRODUCT_DEFINITION', {}).get('status') == 'PASSED':
        return 'COMMITTED'
...
def derive_state(existing: dict | None = None) -> dict:
    ...
    elif phase == 'PRODUCT_DEFINITION' and completeness.get('gates', {}).get('PRODUCT_DEFINITION', {}).get('status') != 'PASSED':
        status = 'WAITING_FOR_INPUT'
        next_action = 'Описать видение, границы и основной пользовательский сценарий; неблокирующие вопросы можно оставить открытыми.'
    elif phase == 'WORKING_PROTOTYPE' and prototype_plan() == 'VISUAL_AND_WORKING' and (
    ...
    elif len(_approved_phase_reviews(reviews) & set(active_phase_sequence())) == len(active_phase_sequence()):
        status = 'VALIDATED'
        next_action = (
            'Handoff завершён. С этого момента системный аналитик или PO переходит к работе над CR, '
            'использует процесс change compiler и формирует требования в '
            'https://github.com/crystalservice/set-requirements. Работа Product Manager завершена.'
        )
    else:
        status = 'IN_PROGRESS'
