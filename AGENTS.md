...
`handoff/` — производное человекочитаемое представление для архитектора, лида и
разработчика. Оно не является источником требований и не используется агентом для
извлечения, проверки, изменения или фиксации требований. Для этих действий агент
читает только `requirements/`, baseline и реестры `product/`.
