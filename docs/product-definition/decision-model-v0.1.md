...
**Связанные артефакты:** [Domain Model v0.1](domain-model-v0.1.md), [Каталог инцидентов v0.1](incident-catalog-v0.1.md), [Нормативный реестр v0.1](regulatory-register-v0.1.md)
...
- признак отношения к розничной реализации/выбытию;
- владелец устранения: Set Mark, SetRetail, ГИС МТ или внешний бизнес-процесс;
- вид инцидента;
...
flowchart TD
    ...
    C -- "Да" --> D{"Возникло при розничной реализации/выбытии?"}
    ...
    F -- "Нет/не подтверждено" --> NF["Риск автоштрафа: нет"]
    NF --> CR["Оценить накопленный регуляторный риск"]
    P --> CR
    O --> CR
    N --> CR
    U --> CR
...
**То:** `RA-05`; риск автоштрафа установить в `Нет`, поскольку подтверждённого применимого основания нет. Сохранить данные и версию правила.

### DM-003. Отклонение вне зоны розничного выбытия

**Если:** тип отклонения не связан с розничной реализацией/выбытием.
...
Если отклонение возникло при розничной реализации/выбытии, оно входит в поддерживаемую область независимо от владельца устранения. Для такого инцидента указывается один или несколько владельцев: Set Mark, SetRetail, ГИС МТ или внешний бизнес-процесс. Внешний владелец устранения сам по себе не приводит к `RA-03`.
...
### DM-004A. Расхождение времени при совпавшем коде идентификации

**Если:** код идентификации из отчёта Честного знака однозначно совпал с данными Set Mark / кассы, но время операции заметно отличается.
**То:** записи связываются по коду идентификации; расхождение времени не блокирует корреляцию и само по себе не приводит к `RA-04`. Оба времени и величина расхождения включаются в объяснение как диагностический факт.
...
**Если:** подтверждённое нарушение соответствует нормативному правилу, действовавшему на дату кассовой операции, с учётом товарной группы, типа операции и вида отклонения. Дата импорта отклонения и дата создания инцидента для применимости нормы не используются.
...
**Иначе:** `potentialAutoFineRisk = false`. Отсутствие подтверждённого применимого основания отображается пользователю как «Риска автоштрафа нет», без промежуточного состояния «Не определён».

### DM-015. Накопление однотипных нарушений

**Если:** для товарной группы, вида отклонения, магазина и календарного месяца существует действующее правило Нормативного реестра; известны локальный числитель и подтверждённый знаменатель — средний объём реализации соответствующей продукции в одном объекте розничной торговли в Российской Федерации за тот же календарный месяц; рассчитанная доля строго превышает нормативный процент.  
**То:** установить критичность агрегированного показателя по магазину. Критичность отдельных инцидентов из-за одного только превышения накопленного порога не изменяется. На каждом инциденте, вошедшем в расчёт, сохраняются признак участия в превышенном агрегате и ссылка на него.

Если национальный знаменатель не получен из подтверждённого внешнего источника, агрегат имеет состояние «Не оценён»: продукт показывает локальное количество и динамику, но не подменяет знаменатель собственным оборотом магазина.

Порог из проекта нормативного акта хранится и отображается отдельно как прогнозный и не изменяет действующую критичность. Процент является индикатором риска для контрольного мероприятия, а не автоштрафом и не допустимым уровнем нарушений. Полный перечень действующих и проектных порогов ведётся в [Нормативном реестре v0.1](regulatory-register-v0.1.md).
...
| Нет | Действующее применимое основание автоштрафа не подтверждено либо условия правила не выполнены |
...
Автоматическая переклассификация существующих инцидентов не выполняется. Каждый инцидент хранит версию правил, применённую при его создании. Новая версия правил применяется только к инцидентам, созданным после её активации.
...
- Рекомендации ограничены текущей версией базовых настроек из Каталога инцидентов. Каталог и правила анализа могут дополняться и исправляться с выпуском новых версий.

## 12. Закрытые вопросы

| ID | Зафиксированное решение |
|---|---|
| OQ-DM-101 | Для v0.1 используются справочники категорий, видов и результатов отклонений из True API v704.0, страницы 1229–1234. После получения JSON `ANALYTICAL_VIOLATIONS_NAMES` справочник выпускается новой версией. |
| OQ-DM-102 | Набор последовательных сценариев проверки из текущей таблицы достаточен для v0.1; новые комбинации добавляются версионируемыми правилами. |
...
| OQ-DM-104 | При совпадении кода идентификации записи связываются независимо от расхождения времени; расхождение показывается в объяснении как диагностический факт. |
| OQ-DM-106 | При превышении накопленного порога критическим становится агрегированный показатель магазина. Отдельные инциденты получают признак участия и ссылку на агрегат, но не становятся критическими по этой причине. |
| OQ-DM-108 | Новая версия нормативных или аналитических правил применяется только к инцидентам, созданным после её активации. Существующие инциденты автоматически не пересчитываются. |
| OQ-DM-109 | Текущие настройки Каталога являются базовым набором рекомендаций v1. Каталог и правила анализа являются развиваемыми версионируемыми артефактами и могут изменяться в ходе создания продукта. |
| OQ-DM-110 | Анализ использует актуальную конфигурацию магазина; если она могла измениться после операции, это ограничение явно указывается в объяснении. |
| OQ-DM-111 | Если подтверждённое применимое основание автоштрафа отсутствует, пользователь видит «Риска автоштрафа нет»; промежуточное состояние неопределённости не используется. |
| OQ-DM-112 | Применимость нормы автоштрафа определяется по дате кассовой операции, а не по дате импорта отклонения или создания инцидента. |
| OQ-DM-105 | Действующие и проектные процентные пороги разделены в Нормативном реестре v0.1; проектные пункты 52–69 не участвуют в действующей критичности. |
| OQ-DM-107 | Составы автоштрафов и даты применения зафиксированы в Нормативном реестре v0.1; правила № 120-ФЗ применяются не ранее 2026-09-01. |

## 13. Открытые вопросы

Открытые нормативные вопросы перенесены в раздел 9 [Нормативного реестра v0.1](regulatory-register-v0.1.md). Главный технический блокер — источник национального среднего объёма реализации для воспроизводимого расчёта процентных индикаторов.
