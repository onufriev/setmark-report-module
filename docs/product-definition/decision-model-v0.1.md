...
flowchart TD
    ...
    D -- "Нет" --> O["RA-03: вне области ответственности"]
    D -- "Да" --> K{"Доступен полный КМ?"}
    K -- "Нет" --> N["RA-04: недостаточно данных"]
    K -- "Да" --> G["Собрать кассовые операции, Set Mark, ЧЗ/ЛМ ЧЗ и конфигурацию Set Centrum"]
...
**То:** `RA-03`, инцидент остаётся видимым, причина исключения объясняется.

### DM-004. Нет полного КМ

**Если:** полный КМ отсутствует или не сопоставляется с данными Set Mark/кассы.
**То:** `RA-04`; GTIN, время, ККТ и магазин не заменяют КМ как штатный ключ.
...
**Если:** последовательность подтверждена как `продажа → корректный возврат → повторная продажа`.
**То:** `RA-02`; повторность сама по себе не подтверждает нарушение.
...
**То:** использование офлайн-режима само по себе не подтверждает нарушение. Итог определяется конкретным типом отклонения и нормативными условиями.

### DM-010. Запрет ЛМ ЧЗ проигнорирован

**Если:** ЛМ ЧЗ вернул запрет, а продажа завершена.
**То:** `RA-01`, если достоверно сопоставлены КМ и операция; нормативная критичность определяется отдельно.

### DM-011. Недоступны онлайн и ЛМ ЧЗ

**Если:** отсутствует результат онлайн-проверки и результат ЛМ ЧЗ.
**То:** результат зависит от действующих нормативных правил и конфигурации магазина. До утверждения такого правила — `RA-04`, с объяснением недоступности проверок.
...
**Если:** число сопоставимых инцидентов одного вида в установленном разрезе и периоде достигает утверждённого порога.
**То:** повысить регуляторный риск и при необходимости критичность группы/инцидента. Порог, период и разрез пока не утверждены, поэтому правило не может применяться автоматически в v0.1.
...
| Онлайн ЧЗ | ЛМ ЧЗ | Решение проверки | Итог кассы | Предварительный результат |
|---|---|---|---|---|
| Доступен | Не используется | Разрешено | Продажа | Определяется видом отклонения; сам сценарий допустим |
| Доступен | Не используется | Запрещено | Продажа | RA-01 |
| Недоступен | Доступен | Разрешено | Продажа | Не считать нарушением только из-за офлайн-режима |
| Недоступен | Доступен | Запрещено | Продажа | RA-01 |
| Недоступен | Недоступен | Нет результата | Продажа | RA-04 до утверждения нормативного правила |
| Любое | Любое | Любое | Продажа не завершена | Инцидент анализируется по True API; состоявшаяся продажа не предполагается без кассового подтверждения |
...
|---|---|
| Не оценён | Пороговые правила ещё не утверждены |
| Низкий | Будет определён после утверждения периода, разреза и порога |
| Повышенный | Будет определён после утверждения периода, разреза и порога |
| Высокий | Будет определён после утверждения периода, разреза и порога |

До утверждения порогов продукт может показывать фактические количества и динамику, но не должен сам назначать уровень регуляторного риска.
...
- Не определены пороги накопленного регуляторного риска.
...
| OQ-DM-102 | Какие комбинации ответа онлайн ЧЗ, ЛМ ЧЗ и кассового итога нормативно допустимы? |
| OQ-DM-103 | Как различить возврат, полностью восстанавливающий допустимость повторной продажи, и возврат, который её не восстанавливает? |
...
| OQ-DM-105 | Какие порог, период и разрез определяют риск проверки из-за однотипных нарушений? |
