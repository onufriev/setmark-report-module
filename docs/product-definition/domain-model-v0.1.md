...
Функция не обращается к API ЛМ ЧЗ за дополнительными или историческими сведениями. Описание API может быть изучено отдельно после его предоставления, но это не расширяет границы v1 автоматически.
...
Признак риска автоштрафа является бинарным. Значение «Да» устанавливается только при наличии подтверждённого применимого нормативного основания. Во всех остальных случаях пользователь видит «Риска автоштрафа нет». Промежуточное значение неопределённости не используется.
...
### 5.13. Агрегированный показатель регуляторного риска магазина

Отдельный объект анализа, рассчитываемый по однотипным инцидентам магазина за установленный период.

При превышении применимого процентного порога:

- критическим становится агрегированный показатель магазина;
- отдельные инциденты не становятся критическими только из-за накопленного риска;
- каждый вошедший в расчёт инцидент получает признак участия в превышенном агрегате и ссылку на него;
- собственная критичность инцидента продолжает определяться его индивидуальными основаниями, включая риск автоштрафа.

### 5.14. Объяснение результата
...
Срок хранения задаётся пользователем торговой сети в настройках продукта. Значение по умолчанию — **90 календарных дней** (`PM-DEC-210`).
...
21. Срок хранения инцидента и связанных с ним данных настраивается; значение по умолчанию составляет 90 календарных дней.
...
23. Та же настройка срока хранения применяется к агрегированной статистике; по умолчанию агрегаты также хранятся 90 календарных дней и затем удаляются.
24. Если онлайн ЧЗ не ответил, fallback через ЛМ ЧЗ также не дал результата, но касса завершила продажу, инцидент получает `RA-01`.
25. Успешно завершённого возврата в SetRetail достаточно для признания последующей повторной продажи допустимой (`RA-02`).
26. Совпадение кода идентификации является достаточным для корреляции даже при расхождении времени; расхождение сохраняется и объясняется.
27. Превышение накопленного регуляторного порога делает критическим агрегированный показатель магазина, но не отдельные инциденты; инциденты получают признак участия и ссылку на агрегат.
28. Изменение правил применяется только к новым инцидентам. Созданные ранее инциденты сохраняют исходную версию правил и автоматически не пересчитываются.
29. Каталог инцидентов, правила анализа и базовые рекомендуемые настройки являются версионируемыми развиваемыми артефактами и могут изменяться в процессе создания продукта.
30. До получения актуального JSON справочника через True API v0.1 использует встроенные справочники из документа True API v704.0.
31. Если подтверждённое применимое основание автоштрафа отсутствует, риск автоштрафа получает значение «Нет»; неопределённое пользовательское состояние не используется.
32. Применимость нормы автоштрафа определяется по дате кассовой операции; дата создания инцидента и дата импорта отклонения не используются.
...
- Функция не контролирует собственным таймером 72-часовой предел автономной работы ЛМ ЧЗ.
...
| OQ-DM-009 | Инцидент, исходный снимок, результат анализа, объяснение и сохранённый контекст обогащения хранятся в течение настраиваемого срока; значение по умолчанию — 90 календарных дней (`PM-DEC-210`). |
...
| OQ-DM-012 | Агрегированная статистика подчиняется той же настройке срока хранения, что и инциденты; значение по умолчанию — 90 календарных дней, после чего агрегаты удаляются (`PM-DEC-210`). |
...
| PM-022 | Срок хранения инцидента настраивается пользователем; актуальное значение по умолчанию — 90 календарных дней (`PM-DEC-210`) |
...
| PM-025 | Продажа без разрешающего результата онлайн ЧЗ и последующего fallback ЛМ ЧЗ является подтверждённым нарушением `RA-01` |
| PM-026 | Успешно завершённый возврат в SetRetail достаточен для признания последующей повторной продажи допустимой (`RA-02`) |
| PM-027 | Расхождение времени не блокирует корреляцию при совпадении кода идентификации и отражается в объяснении |
| PM-028 | Накопленный регуляторный риск повышает критичность агрегата магазина; отдельные инциденты только помечаются как вошедшие в расчёт |
| PM-029 | Новые правила применяются только к новым инцидентам; существующие инциденты автоматически не переклассифицируются |
| PM-030 | Текущие Каталог инцидентов, правила анализа и рекомендуемые настройки являются базовой версией v0.1 и могут изменяться в процессе создания продукта |
| PM-031 | Для v0.1 используются справочники отклонений из True API v704.0; актуальный JSON будет подключён позднее новой версией |
| PM-032 | При отсутствии подтверждённого применимого основания продукт показывает «Риска автоштрафа нет» |
| PM-033 | Применимость нормы автоштрафа определяется по дате кассовой операции |
| PM-034 | В область продукта входят все отклонения розничной реализации/выбытия; внешний контур устранения не исключает инцидент из анализа, а указывается отдельно как Set Mark, SetRetail, ГИС МТ или внешний бизнес-процесс |
